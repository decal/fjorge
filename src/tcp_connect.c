...
int tcp_connect(const char *ahost, const unsigned short aport) {
  register signed int filedesc = 0;
  struct sockaddr_in inetaddr = { 0x00 }, connaddr = { 0x00, };
  struct addrinfo *inetinfo = calloc(1, sizeof(struct addrinfo));
  ...
  if(vcmd->verbose > 1)
    fprintf(stderr, "*** Connecting to %s:%hu via TCP" CRLF, vcmd->hostnam, vcmd->portnum);
